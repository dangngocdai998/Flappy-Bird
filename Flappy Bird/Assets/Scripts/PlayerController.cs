...
public class PlayerController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (animator && animControllers.Length > 0)
        {
            animator.runtimeAnimatorController = animControllers[Random.Range(0, animControllers.Length)];
        }
    ...
    void LateUpdate()
    {
        if (GameManager.Instance.GameState())
        {
            if (Input.GetMouseButtonDown(0))
            {
                ...
                playerRigid.velocity = new Vector2(0, thrust);
        ...
        if (playerRigid.velocity.y < -1f)
        {
            ...
            tiltSmooth = maxTiltSmooth;
            playerRigid.velocity = new Vector2(0, playerRigid.velocity.y - (thrust + 3) * Time.deltaTime);
            // playerRigid.gravityScale = 2f;
            // playerRigid.mass = 1000;
        }
    }
